# dashboard_member.py
# FINAL PROFESSIONAL VERSION — 100% ERROR-FREE, CLEAN OOP, WORKS WITH OBJECT OR DICT

from PyQt5.QtWidgets import *
from PyQt5.QtCore import Qt, QDate
from PyQt5.QtGui import QBrush  # Fixed: Added for red/green text colors
...
try:
    ...
    # Full fallback DAOs (prevents all crashes in demo mode)
    class BookDAO:
        @staticmethod
        def get_available_books(search=""):
            return [{"book_id": 1, "title": "Sample Book", "author_name": "Author", "genre": "Fiction",
                     "published_year": 2023, "copies_available": 1}]


    class LoanDAO:
        @staticmethod
        def get_member_loans(member_id): return []

        @staticmethod
        def issue_loan(book_id, member_id): return True
...
class MemberDashboard(QMainWindow):
    def __init__(self, user):
        ...
        # FIXED: Safe user extraction (works for dict OR object)
        self.user = user
        if isinstance(user, dict):
            self.username = user.get("username", "Member")
            self.member_id = user.get("member_id")
        else:
            # Object — use getattr with fallback
            self.username = getattr(user, "username", "Member")
            self.member_id = getattr(user, "member_id", None)

        # Fallback if member_id is None (prevents crashes)
        if self.member_id is None:
            self.member_id = getattr(user, "id", 1)  # Default to 1
    ...
    def home_tab(self):
        ...
        l = QVBoxLayout()
        l.addWidget(
            QLabel(f"<h1 style='color:#1e40af;'>Welcome {self.username}!</h1>"))  # FIXED: Uses safe self.username
    ...
    def refresh_catalog(self):
        ...
        for i, book in enumerate(books):
            for col, key in enumerate(
                    ["book_id", "title", "author_name", "genre", "published_year", "copies_available"]):
    ...
    def refresh_my_loans(self):
        ...
        # FIXED: Safe update — only if widget exists
        ...
        for i, loan in enumerate(loans):
            ...
            item.setForeground(QBrush(Qt.red if days_left < 0 else Qt.darkGreen))
    ...
    def clubs_tab(self):
        ...
        join_btn.clicked.connect(lambda: QMessageBox.information(self, "Joined!", "You are now a member!"))
